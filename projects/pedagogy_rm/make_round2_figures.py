...
def main() -> None:
    ...
    for i, v in enumerate(per_kl):
        right.text(i, v + 0.13, f"{v:.2f}", ha="center", fontsize=8.5,
                   fontweight="bold" if i == 1 else "normal")
    right.set_ylim(0, max(per_kl) * 1.2)
    right.annotate("chosen", (1, per_kl[1]), textcoords="offset points", xytext=(0, 26),
                   ha="center", fontsize=8.5, color="#2f6db5", fontweight="bold",
                   arrowprops={"arrowstyle": "->", "color": "#2f6db5"})
    right.annotate("arm B's regime:\nmore reward, worse rate", (3, per_kl[3]),
                   textcoords="offset points", xytext=(-4, 34), ha="center", fontsize=8,
                   color="#c0392b", arrowprops={"arrowstyle": "->", "color": "#c0392b"})
    fig.suptitle("Learning rate: the two rankings disagree", fontsize=12.5, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(os.path.join(args.figures, "lr_efficiency.png"), dpi=150, bbox_inches="tight",
                facecolor="white")
    plt.close(fig)
    ...
    ax.text(4.55, 0.24, "0.4 — below this a\ndimension is discarded",
            fontsize=8, color="#c0392b", ha="center",
            bbox={"boxstyle": "round,pad=0.3", "facecolor": "white", "edgecolor": "#c0392b",
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(args.figures, "rubric_v2.png"), dpi=150, bbox_inches="tight",
                facecolor="white")
    plt.close(fig)
    ...
    fig.suptitle("Rating one dimension at a time decoupled the scales",
                 fontsize=12.5, fontweight="bold", y=1.0)
    fig.savefig(os.path.join(args.figures, "decoupling.png"), dpi=150, bbox_inches="tight",
                facecolor="white")
    plt.close(fig)
    ...
    ax.set_ylim(0, 1.14)
    ax.set_title("Two attempts to decouple leak from length; the second made it worse",
                 fontsize=10.5)
    # Above the bar rather than beside it: at 0.74 the bar fills the plot and any offset
    # to the side lands on top of it.
    ax.annotate("overfit: alpha fell to 0.1,\nspread rose 0.46 to 0.64", (3, 0.755),
                textcoords="offset points", xytext=(-118, 30), ha="center", fontsize=8,
                color="#c0392b", arrowprops={"arrowstyle": "->", "color": "#c0392b"})
    bars[0].set_hatch("//")
    fig.tight_layout()
    fig.savefig(os.path.join(args.figures, "leak_repairs.png"), dpi=150, bbox_inches="tight",
                facecolor="white")
    plt.close(fig)
