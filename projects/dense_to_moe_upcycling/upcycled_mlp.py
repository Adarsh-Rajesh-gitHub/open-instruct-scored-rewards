...
class UpcycledQwenMLP(nn.Module):
    ...
    def __init__(self, dense_mlp: nn.Module, num_experts: int = 4, top_k: int = 2, router_std: float = 0.02):
        super().__init__()
        if num_experts < 2:
        ...
        self.router = nn.Linear(hidden_size, num_experts, bias=True, device=reference.device, dtype=reference.dtype)
        nn.init.normal_(self.router.weight, mean=0.0, std=router_std)
        nn.init.zeros_(self.router.bias)
        self.register_buffer(
            "cumulative_counts", torch.zeros(num_experts, dtype=torch.long, device=reference.device), persistent=False
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.top_k == 1:
            ...
            weights = torch.cat((weights[:, :-1], 1.0 - weights[:, :-1].sum(dim=-1, keepdim=True)), dim=-1)
        output = torch.zeros_like(flat, dtype=torch.float32)

        for expert_index, expert in enumerate(self.experts):
            token_indices, slots = torch.where(top_indices == expert_index)
            if token_indices.numel() == 0:
                continue
            expert_output = expert(flat[token_indices])
            weighted = expert_output.float() * weights[token_indices, slots, None]
    ...
    def router_losses(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        top_one = torch.nn.functional.one_hot(self._last_top_indices[:, 0], num_classes=self.num_experts).to(
            probabilities.dtype
        )
        load_balance = self.num_experts * torch.sum(probabilities.mean(dim=0) * top_one.mean(dim=0))
        z_loss = torch.mean(torch.logsumexp(self._last_router_logits, dim=-1).square())
    ...
    def routing_fractions(self) -> list[float]:
        ...
        return [float(value / total) for value in self.cumulative_counts.tolist()]
...
def upcycle_qwen_layers(
    model: nn.Module, layer_indices: Iterable[int], num_experts: int = 4, top_k: int = 2, router_std: float = 0.02
) -> dict:
    ...
    for index in indices:
        ...
        layers[index].mlp = UpcycledQwenMLP(
            layers[index].mlp, num_experts=num_experts, top_k=top_k, router_std=router_std
        )
    return {"layer_indices": indices, "num_experts": num_experts, "top_k": top_k, "router_std": router_std}
...
def combined_router_loss(model: nn.Module) -> tuple[torch.Tensor, torch.Tensor]:
...
def routing_report(model: nn.Module) -> dict[str, list[float]]:
    return {str(index): module.routing_fractions() for index, module in iter_upcycled_layers(model)}


def set_only_moe_trainable(model: nn.Module, train_experts: bool = True, train_router: bool = True) -> dict[str, int]:
...
def save_moe_checkpoint(
    model: nn.Module, path: Path, base_model: str, manifest: dict, extra: dict | None = None
) -> None:
    modules = {str(index): module.state_dict() for index, module in iter_upcycled_layers(model)}
    payload = {"base_model": base_model, "manifest": manifest, "moe_modules": modules, "extra": extra or {}}
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(payload, path)
    path.with_suffix(".json").write_text(
        json.dumps({"base_model": base_model, "manifest": manifest, "extra": extra or {}}, indent=2) + "\n"
    )
