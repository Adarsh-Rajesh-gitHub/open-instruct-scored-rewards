...
from projects.dense_to_moe_upcycling.upcycled_mlp import iter_upcycled_layers, restore_moe_checkpoint, routing_report
...
def main() -> None:
    ...
    model = AutoModelForCausalLM.from_pretrained(args.model, dtype=torch.bfloat16, attn_implementation="sdpa").to(
        "cuda"
    )
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a precise teacher. Diagnose the learner's reasoning and choose an instructional response."
            ),
        },
