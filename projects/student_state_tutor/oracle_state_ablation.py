...
def load_examples(path: Path, limit: int, seed: int) -> list[dict]:
    ...
    for line in path.read_text().splitlines():
        ...
        if belief_idx == gold_idx or question in seen_questions:
            continue
        counterfactual_indices = [index for index in range(len(choices)) if index not in (gold_idx, belief_idx)]
...
def generate_tutor_responses(args, examples: list[dict]) -> list[dict]:
    ...
    for start in range(0, len(requests), args.generation_batch_size):
        ...
        for (example_index, condition, _), response in zip(batch, decoded, strict=True):
    ...
    return [
        {**example, "tutor_responses": response_map} for example, response_map in zip(examples, responses, strict=True)
    ]
...
def score_records(args, records: list[dict]) -> list[list[float]]:
    ...
    if tokenizer.pad_token_id is None:
        tokenizer.pad_token_id = tokenizer.eos_token_id
    model = AutoModelForCausalLM.from_pretrained(args.student_model, dtype=torch.bfloat16).to(args.device)
    model.eval()
    ...
    for record_index, record in enumerate(records):
        ...
        for choice_index, choice in enumerate(record["choices"]):
            ...
            flattened.append((record_index, choice_index, prompt_ids + choice_ids, len(prompt_ids)))
    ...
    for start in range(0, len(flattened), args.scoring_batch_size):
        ...
        input_ids = torch.full((len(batch), max_length), tokenizer.pad_token_id, dtype=torch.long, device=args.device)
        ...
        for row_index, (_, _, sequence, prompt_length) in enumerate(batch):
            ...
            input_ids[row_index, offset:] = torch.tensor(sequence, dtype=torch.long, device=args.device)
        ...
        sequence_scores = (token_log_probabilities * shifted_choice_mask).sum(dim=1) / shifted_choice_mask.sum(
            dim=1
        ).clamp_min(1)
        for (record_index, choice_index, _, _), score in zip(batch, sequence_scores.tolist(), strict=True):
            scored[record_index][choice_index] = score
        print(f"scored {min(start + len(batch), len(flattened))}/{len(flattened)} choices", flush=True)
    return scored
...
def leaks_answer(response: str, example: dict) -> bool:
    ...
    if len(gold) >= 4 and gold in normalized_response:
        return True
    return bool(re.search(r"\b(?:answer|option|choice)\s*(?:is\s*)?[abcd]\b", response, flags=re.IGNORECASE))
...
def paired_difference(rows: list[dict], left: str, right: str, metric: str) -> np.ndarray:
    return np.asarray(
        [row["condition_scores"][left][metric] - row["condition_scores"][right][metric] for row in rows],
        dtype=np.float64,
    )
...
def summarize(rows: list[dict], seed: int) -> dict:
    ...
    for left, right in (("oracle", "transcript"), ("oracle", "counterfactual"), ("transcript", "counterfactual")):
...
def main() -> None:
    ...
    if generations_path.exists() and not args.force_generation:
        ...
        print(
            f"loaded {len(examples)} examples: {dict(Counter(example['subject'] for example in examples))}", flush=True
        )
    ...
    score_records_input = []
    for row in rows:
        score_records_input.append(make_score_record(row["question"], row["choices"], ""))
        for condition in CONDITIONS:
            score_records_input.append(
                make_score_record(row["question"], row["choices"], row["tutor_responses"][condition])
            )
