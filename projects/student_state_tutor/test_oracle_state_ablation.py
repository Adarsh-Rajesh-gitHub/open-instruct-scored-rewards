...
def test_extract_student_opening():
    ...
    assert (
        oracle_state_ablation.extract_student_opening(completion)
        == "I thought current gets used up."
    )
...
def test_answer_leak_detection():
    example = {
        "choices": ["clouds", "Earth spinning on its axis", "the Moon", "wind"],
        "gold_idx": 1,
    }
    assert oracle_state_ablation.leaks_answer(
        "The answer is B.", example
    )
    assert oracle_state_ablation.leaks_answer(
        "Think about Earth spinning on its axis.", example
    )
