...
def test_build_cases_has_known_weakest_skill():
    cases = teacher_action_ablation.build_cases(
        [bank(index) for index in range(4)], replicates=2, seed=0
    )
    assert len(cases) == 8
    for case in cases:
        node = next(
            node
            for node in case["graph_view"]["nodes"]
            if node["concept_id"] == case["oracle_target"]
        )
        assert node["mean"] == min(
            row["mean"] for row in case["graph_view"]["nodes"]
        )
        assert case["oracle_action"] in teacher_action_ablation.ACTIONS


def test_parse_decision_validates_target_item_pair():
    case = teacher_action_ablation.build_cases(
        [bank(index) for index in range(4)], replicates=1, seed=0
    )[0]
    target = case["oracle_target"]
    item_id = next(
        row["item_id"] for row in case["candidates"]
...
def test_summary_passes_for_oracle_decisions():
    ...
    for case in cases:
        ...
        for condition in teacher_action_ablation.CONDITIONS:
            target = (
                case["shuffled_visible_target"]
                if condition == "shuffled_graph"
                else case["oracle_target"]
            )
            item_id = next(
                row["item_id"] for row in case["candidates"]
                if row["concept_id"] == target
