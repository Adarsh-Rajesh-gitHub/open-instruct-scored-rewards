...
def domain_state_matrices(
    ...
    for domain in range(n_domains):
        ...
        history_state[:, domain] = sphere.posterior_mean(
            correctness[:, history], axis=1
        )
        heldout_state[:, domain] = sphere.posterior_mean(
            correctness[:, target], axis=1
        )
...
def run_seed(correctness: np.ndarray, domains: np.ndarray, seed: int) -> dict:
    ...
    history_state, heldout_state = domain_state_matrices(
        correctness, domains, history_items, target_items
    )
    ...
    shuffled_state = history_state[
        :, rng.permutation(len(sphere.INSTRUMENTS))
    ]
    ...
    return {
        ...
        "shuffled_minus_selected": float(
            (shuffled_selected - selected).mean()
        ),
        "strong_minus_weak": float(
            (strongest_selected - selected).mean()
        ),
    ...
    return {
        ...
        "metrics": {
            metric: {
                ...
                "95_interval_across_item_splits": interval(
...
def main() -> None:
    ...
    results = [
        run_seed(correctness, domains, seed) for seed in range(args.seeds)
    ]
    summary = summarize(
        results, students=len(correctness), items=len(item_names)
    )
