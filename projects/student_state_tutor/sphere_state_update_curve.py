...
def split_with_k_history(
    domains: np.ndarray, k: int, rng: np.random.Generator
...
def run(
    ...
    history_state, heldout_state = curriculum.domain_state_matrices(
        correctness, domains, history, target
    )
    ...
    for k in ks:
        ...
        by_k[str(k)] = {
            metric: {
                ...
                "95_interval_across_item_splits": curriculum.interval(
...
def main() -> None:
    ...
    results = [
        run(correctness, domains, k, seed)
        for k in args.history_counts
