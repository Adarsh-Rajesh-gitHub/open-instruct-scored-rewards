"""Needs pyreadr, which the trainer's dependencies do not include.
...
def test_student_states_are_domain_specific():
    correctness = np.asarray([[1, 1, 0, 0, 1, 0], [0, 0, 1, 1, 0, 1]], dtype=np.float32)
...
def test_item_split_keeps_history_in_every_domain():
    ...
    history, target = sphere_mastery_prediction.split_items(domains, np.random.default_rng(0))
