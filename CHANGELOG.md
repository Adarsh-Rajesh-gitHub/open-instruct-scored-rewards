...
### Fixed
- Skip the `dist.barrier()` and `dist.all_reduce()` calls in `build_reference_logprobs_cache` when no process group exists, so a single-process DPO run reaches training instead of dying with `Default process group has not been initialized` (https://github.com/edu-llm/open-instruct-scored-rewards/pull/11).
